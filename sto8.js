...
if (!fs.existsSync('uploadx2')) {
    fs.mkdirSync('uploadx2');
}
...
// Serve the HTML file
app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'sto8.html'));
});
...
app.put('/api/upload', (req, res) => {
    ...
    const filePath = path.join(__dirname, 'uploadx2', `${STREAM_NAME}_chunk_${CHUNK_NUMBER}.webm`);
